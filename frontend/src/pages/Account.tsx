...
import type { AuthUser, Booking, Pet, PetParentProfile } from "../lib/types";
...
export default function Account() {
  ...
  const parentProfile = profile as PetParentProfile | null;
  const navigate = useNavigate();
  const [searchParams, setSearchParams] = useSearchParams();
  const [justBooked, setJustBooked] = useState(searchParams.get("booked") === "1");

  useEffect(() => {
    if (searchParams.has("booked")) setSearchParams({}, { replace: true });
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);

  const [pets, setPets] = useState<Pet[]>(parentProfile?.pets ?? []);
    ...
    <PetParentLayout active="profile">
      <header className="flex items-center justify-between border-b border-outline-variant/30 bg-surface-container-lowest px-container-padding-mobile py-6 md:px-container-padding-desktop">
        ...
        <div className="relative" ref={menuRef}>
          ...
          {menuOpen && (
            <div className="soft-card-shadow absolute right-0 top-12 z-20 w-56 rounded-xl border border-outline-variant bg-surface-container-lowest py-2">
              <button
                onClick={() => {
                  setMenuOpen(false);
                  setEditOpen(true);
      ...
      <main className="flex flex-col gap-section-gap px-container-padding-mobile py-6 md:px-container-padding-desktop">
        {justBooked && (
          <div className="flex items-center justify-between rounded-lg border border-secondary/30 bg-secondary/10 px-4 py-3 font-body-md text-body-md text-secondary">
        ...
        <section>
          ...
          {pets.length === 0 ? (
            <p className="font-body-md text-body-md text-on-surface-variant">No pets yet.</p>
          ) : (
            <div className="grid gap-4 sm:grid-cols-2">
              {pets.map((pet) => (
                <div
                  key={pet.id}
                  className="soft-card-shadow flex items-center justify-between rounded-xl border border-outline-variant bg-surface-container-lowest p-4"
                >
          ...
          <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
            ...
            {pets.length > 0 && (
              <select
                ...
                className="rounded-lg border border-outline-variant bg-surface-container-lowest px-3 py-2 font-body-md text-body-md text-on-surface"
              >
      ...
      {deletePet && (
        <ConfirmDialog
          ...
          message="This removes them from your account. Their medical records are kept but the pet profile can't be undone."
          confirming={busy}
      ...
      {deleteAccountOpen && (
        <ConfirmDialog
          ...
          message="This permanently deletes your account, pets, and records. This can't be undone."
...
function EditAccountModal({
  ...
  onClose: () => void;
  onSaved: (user: AuthUser, profile: PetParentProfile) => void;
}) {
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    setSaving(true);
    try {
      const data = await api.patch<{ user: AuthUser; profile: PetParentProfile }>("/auth/me", {
        name,
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-inverse-surface/40 px-4">
