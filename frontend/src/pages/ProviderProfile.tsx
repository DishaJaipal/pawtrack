...
import type { AvailableSlot, Booking, DiscoveryProvider, Pet, Service } from "../lib/types";
...
export default function ProviderProfile() {
  ...
  const [provider, setProvider] = useState<DiscoveryProvider | null>(null);
  ...
  useEffect(() => {
    ...
    setLoading(true);
    api
      .get<DiscoveryProvider>(`/providers/${providerId}`)
      .catch((err) => {
  ...
  const loadSlots = useCallback(
    async (service: Service) => {
      ...
      try {
        const data = await api.get<AvailableSlot[]>(`/providers/${providerId}/services/${service.id}/slots`);
        setSlots(data);
      } finally {
      ...
      <div className="mx-auto max-w-2xl pb-16">
        ...
        <section className="mt-section-gap px-container-padding-mobile">
          ...
          {provider.services.length === 0 ? (
            <p className="font-body-md text-body-md text-on-surface-variant">No services listed yet.</p>
          ) : (
                    ...
                    <div className="flex flex-wrap gap-2">
                      {daySlots.map((slot) => (
                        <button
                          ...
                          className="flex flex-col items-center rounded-2xl border border-outline-variant/50 bg-white px-4 py-2 transition-colors hover:border-secondary"
                        >
...
function ConfirmBookingModal({
  ...
  return (
    <Modal title="Confirm Booking" onClose={onClose}>
      <div className="flex flex-col gap-4">
        ...
        {pets.length === 0 ? (
          ...
          <FormSelect id="pet" label="Booking for" value={petId} onChange={(e) => setPetId(e.target.value)}>
            {pets.map((p) => (
