...
export default function FindAProvider() {
  ...
  const load = useCallback(async () => {
    ...
    const data = await api.get<DiscoveryProvider[]>(`/providers${query ? `?${query}` : ""}`);
    setProviders(data);
  }, [search, category]);
    ...
    <PetParentLayout active="search">
      <div className="mx-auto max-w-4xl px-container-padding-mobile py-6 md:px-container-padding-desktop">
        ...
        <div className="relative">
          ...
          <input
            className="h-14 w-full rounded-2xl border-none bg-surface-container-low pl-12 pr-4 font-body-md text-body-md shadow-sm outline-none placeholder:text-on-surface-variant/60 focus:ring-2 focus:ring-secondary"
        ...
        <div className="hide-scrollbar mt-4 flex gap-2 overflow-x-auto pb-2">
          <button
            ...
            className={`font-label-md text-label-md whitespace-nowrap rounded-full px-4 py-2 transition-colors ${
              category === null ? "bg-secondary text-white" : "bg-surface-container text-on-surface-variant"
            }`}
          >
            All
          </button>
          {PROVIDER_TYPES.map((t) => (
            <button
              ...
              className={`font-label-md text-label-md whitespace-nowrap rounded-full px-4 py-2 transition-colors ${
                category === t.value ? "bg-secondary text-white" : "bg-surface-container text-on-surface-variant"
              }`}
            >
              {t.label}
            </button>
          ...
          {providers.map((p) => {
            ...
            return (
              <Link
                key={p.id}
                to={`/providers/${p.id}`}
                className="soft-card-shadow flex gap-4 rounded-3xl border border-outline-variant/10 bg-surface-container-lowest p-5 transition-all hover:-translate-y-1"
              >
                ...
                <div className="flex flex-1 flex-col justify-between">
                  ...
                  <div className="mt-3 flex flex-wrap gap-2">
                    {categories.slice(0, 3).map((c) => (
                      <span key={c} className="font-label-sm text-label-sm rounded-full bg-secondary/10 px-3 py-1 text-secondary">
                        {c}
                      </span>
                    ))}
                    {categories.length === 0 && (
                      <span className="font-label-sm text-label-sm text-on-surface-variant">No services listed yet</span>
                    )}
                  </div>
