...
import type { AuthUser, PetParentProfile } from "../../lib/types";
...
const emptyAccount: AccountDetails = { name: "", email: "", password: "", phoneNo: "", address: "" };
const emptyPet: PetDetails = { name: "", species: "", breed: "", ageMonths: "", weightKg: "", allergies: "" };
...
export default function SignupPetParent() {
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    setSubmitting(true);
    try {
      const data = await api.post<{ user: AuthUser; profile: PetParentProfile }>("/auth/register", {
        role: "PET_PARENT",
        name: account.name,
        email: account.email,
        ...
        {step === 1 ? (
          <form onSubmit={handleNext} className="mt-6 flex flex-col gap-4">
            ...
            <FormField
              ...
              onChange={(e) => setAccount({ ...account, phoneNo: e.target.value })}
            />
            <FormField
              id="address"
              label="Address"
          ...
          <form onSubmit={handleSubmit} className="mt-6 flex flex-col gap-4">
            ...
            <SpeciesBreedPicker
              species={pet.species}
              breed={pet.breed}
              onChange={(species, breed) => setPet({ ...pet, species, breed })}
            />
            <div className="grid grid-cols-2 gap-4">
              ...
              <FormField
                ...
                step="0.1"
                value={pet.weightKg}
                onChange={(e) => setPet({ ...pet, weightKg: e.target.value })}
              />
            </div>
            <FormField
              id="allergies"
