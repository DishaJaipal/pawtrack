...
const CATEGORIES: { value: RecordType | "ALL"; label: string }[] = [
  { value: "ALL", label: "All" },
...
const RECORD_ICON: Record<RecordType, string> = {
...
export default function PetProfile() {
  ...
  const [activeTab, setActiveTab] = useState<"details" | "records">(
    searchParams.get("tab") === "records" ? "records" : "details"
  );
  const [category, setCategory] = useState<RecordType | "ALL">("ALL");
  ...
  const loadRecords = useCallback(async () => {
    ...
    const data = await api.get<PetRecord[]>(`/pets/${petId}/records${query ? `?${query}` : ""}`);
    setRecords(data);
  ...
  useEffect(() => {
    if (loading) return;
    if (range === "custom" && !customFrom && !customTo) return; // wait for at least one bound
    ...
    <PetParentLayout active="home">
      <div className="pb-8 font-body-md text-on-surface">
        ...
        <header className="relative flex h-[280px] w-full flex-col justify-end overflow-hidden bg-gradient-to-br from-primary to-primary-container p-container-padding-mobile">
          <button
            onClick={() => navigate(-1)}
            className="absolute left-4 top-4 z-20 flex h-10 w-10 items-center justify-center rounded-full bg-white/20 text-white backdrop-blur-md active:scale-90"
          >
          ...
          <div className="z-10 flex items-end justify-between text-white">
            ...
            <button
              ...
              className="flex h-12 w-12 items-center justify-center rounded-xl bg-primary text-white shadow-lg active:scale-95"
            >
        ...
        <nav className="sticky top-0 z-30 bg-background/80 px-container-padding-mobile py-4 shadow-sm backdrop-blur-xl">
          <div className="flex space-x-1 rounded-2xl bg-surface-container p-1">
            <button
              ...
              className={`font-label-md text-label-md flex-1 rounded-xl py-3 text-center transition-all ${
                activeTab === "details" ? "bg-white text-secondary shadow-sm" : "text-on-surface-variant"
              }`}
            >
              Details
            </button>
            <button
              ...
              className={`font-label-md text-label-md flex-1 rounded-xl py-3 text-center transition-all ${
                activeTab === "records" ? "bg-white text-secondary shadow-sm" : "text-on-surface-variant"
              }`}
            >
              Records
            </button>
        ...
        <main className="px-container-padding-mobile pt-gutter">
          {activeTab === "details" ? (
                ...
                {alerts.length > 0 ? (
                  <div className="flex flex-wrap gap-2">
                    {alerts.map((alert) => (
                      <span
                        key={alert}
                        className="font-label-md text-label-md rounded-full bg-error/10 px-4 py-2 text-error"
                      >
                        {alert}
                      </span>
                    ))}
            ...
            <section className="space-y-4">
              <div className="hide-scrollbar flex gap-2 overflow-x-auto pb-2">
                {CATEGORIES.map((c) => (
                  <button
                    ...
                    className={`font-label-md text-label-md whitespace-nowrap rounded-full px-4 py-2 transition-colors ${
                      category === c.value
              ...
              <div className="hide-scrollbar flex gap-4 overflow-x-auto border-b border-outline-variant/20 pb-2">
                {RANGE_OPTIONS.map((r) => (
                  <button
                    ...
                    className={`text-label-md font-label-md whitespace-nowrap pb-2 transition-colors ${
                      range === r.value
              ...
              {range === "custom" && (
                <div className="flex flex-wrap gap-4 rounded-xl bg-surface-container p-4">
                  <FormField
                    id="customFrom"
                    label="From"
                    type="date"
              ...
              {records.length === 0 ? (
                <p className="font-body-md text-body-md py-8 text-center text-on-surface-variant">
                  ...
                  {records.map((record) => (
                    <div key={record.id} className="relative flex items-start gap-4">
                      ...
                      <div className="flex-1 rounded-2xl border border-outline-variant/20 bg-white p-4 shadow-sm">
                        ...
                        {record.description && (
                          <p className="font-body-md text-body-md mb-1 text-on-surface-variant">
                            {record.description}
                          </p>
                        )}
                        {record.fileUrl && (
                          <a
                            ...
                            target="_blank"
                            rel="noreferrer"
                            className="font-label-sm text-label-sm inline-flex items-center gap-1 text-secondary"
                          >
        ...
        <div className="fixed bottom-24 right-6 z-40">
          <button
            ...
            className="flex items-center gap-2 rounded-3xl bg-primary py-4 pl-4 pr-6 text-white shadow-lg transition-all active:scale-90"
          >
...
function EditPetModal({
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    setSaving(true);
    try {
      const updated = await api.put<Pet>(`/pets/${pet.id}`, {
        name: form.name,
...
function UploadRecordModal({
  ...
  petId: string;
  onClose: () => void;
  onUploaded: (record: PetRecord) => void;
}) {
  const [form, setForm] = useState({
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      ...
      const record = await api.upload<PetRecord>(`/pets/${petId}/records`, formData);
      onUploaded(record);
    } catch (err) {
  ...
  return (
    <Modal title="Upload Record" onClose={onClose}>
      <form onSubmit={handleSubmit} className="flex flex-col gap-4">
        <ErrorBanner message={error} />
        <FormSelect
          id="recordType"
          ...
          onChange={(e) => setForm({ ...form, recordType: e.target.value as RecordType })}
        >
