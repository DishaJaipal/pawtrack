...
import type { Service, ServiceCategory, StaffMember, StaffRole, TimeSlot } from "../lib/types";
...
const STAFF_ROLES: StaffRole[] = ["VET", "GROOMER", "TRAINER", "ASSISTANT", "RECEPTIONIST"];
...
export default function ProviderServices() {
  ...
  const loadAll = useCallback(async () => {
    const [s, st] = await Promise.all([
      api.get<Service[]>("/providers/me/services"),
      api.get<StaffMember[]>("/providers/me/staff"),
    ]);
    setServices(s);
    setStaff(st);
  }, []);
  ...
  async function toggleServiceActive(service: Service) {
    ...
    try {
      ...
      const updated = await api.patch<Service>(`/providers/me/services/${service.id}/${action}`);
  ...
  async function toggleStaffActive(member: StaffMember) {
    ...
    try {
      ...
      const updated = await api.patch<StaffMember>(`/providers/me/staff/${member.id}/${action}`);
  ...
  if (loading) {
    return (
      <ProviderLayout active="services">
    ...
    <ProviderLayout active="services">
      ...
      <nav className="sticky top-0 z-10 bg-background/90 px-container-padding-mobile py-4 backdrop-blur-xl md:px-container-padding-desktop">
        <div className="flex max-w-md space-x-1 rounded-2xl bg-surface-container p-1">
          {(["services", "staff", "slots"] as Tab[]).map((t) => (
            <button
              key={t}
              onClick={() => setTab(t)}
              className={`font-label-md text-label-md flex-1 rounded-xl py-3 text-center capitalize transition-all ${
                tab === t ? "bg-white text-secondary shadow-sm" : "text-on-surface-variant"
              }`}
            >
              {t}
            </button>
      ...
      <main className="px-container-padding-mobile pb-12 md:px-container-padding-desktop">
        ...
        {tab === "services" && (
          <section className="max-w-2xl space-y-4">
            ...
            {services.length === 0 ? (
              <p className="font-body-md text-body-md py-8 text-on-surface-variant">
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                {services.map((s) => {
                  ...
                  return (
                    <div
                      key={s.id}
                      className="soft-card-shadow rounded-xl border border-outline-variant bg-surface-container-lowest p-5"
                    >
                      <div className="flex items-start justify-between">
                        ...
                        {inactiveReason && (
                          <span className="font-label-sm text-label-sm rounded-full bg-surface-container-high px-3 py-1 text-on-surface-variant">
                            {inactiveReason}
                          </span>
                        )}
                      </div>
                      {s.description && (
                        <p className="font-body-md text-body-md mt-2 text-on-surface-variant">{s.description}</p>
                      )}
                      ...
                      <div className="mt-3 flex flex-wrap gap-4">
                        ...
                        <button
                          ...
                          className="font-label-sm text-label-sm text-on-surface-variant disabled:opacity-50"
                        >
        ...
        {tab === "staff" && (
          <section className="max-w-2xl space-y-4">
            {services.filter((s) => s.isActive).length === 0 ? (
              <p className="font-body-md text-body-md rounded-xl bg-surface-container p-4 text-on-surface-variant">
            ...
            {staff.length === 0 ? (
              <p className="font-body-md text-body-md py-8 text-on-surface-variant">No staff added yet.</p>
            ) : (
              <div className="grid gap-4 sm:grid-cols-2">
                {staff.map((member) => (
                  <div
                  ...
                  >
                    ...
                    <div className="mt-3 flex flex-wrap gap-2">
                      {member.staffServices?.map(({ service }) => (
                        <span
                          key={service.id}
                          className="font-label-sm text-label-sm rounded-full bg-secondary/10 px-3 py-1 text-secondary"
                        >
                          {service.name}
                        </span>
                      ))}
      ...
      {deleteService && (
        <ConfirmDialog
          ...
          message="This can't be undone. If this service has ever been booked, you'll need to deactivate it instead."
          confirming={busyId === deleteService.id}
      ...
      {editStaff && (
        <StaffFormModal
          staff={editStaff}
          services={services.filter((s) => s.isActive || editStaff.staffServices?.some((ss) => ss.service.id === s.id))}
      ...
      {deleteStaff && (
        <ConfirmDialog
          title={`Delete ${deleteStaff.name}?`}
          message="This can't be undone. If this staff member has ever been booked, deactivate them instead."
          confirming={busyId === deleteStaff.id}
...
function ServiceFormModal({
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      ...
      const saved = isEdit
        ? await api.put<Service>(`/providers/me/services/${service!.id}`, payload)
        : await api.post<Service>("/providers/me/services", payload);
      onSaved(saved);
    } catch (err) {
...
function StaffFormModal({
  ...
  const [role, setRole] = useState<StaffRole>(staff?.role ?? STAFF_ROLES[0]);
  const [serviceIds, setServiceIds] = useState<string[]>(
    staff?.staffServices?.map((ss) => ss.service.id) ?? []
  );
  const [error, setError] = useState<string | null>(null);
  const [saving, setSaving] = useState(false);
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      const saved = isEdit
        ? await api.put<StaffMember>(`/providers/me/staff/${staff!.id}`, { name, role, serviceIds })
        : await api.post<StaffMember>("/providers/me/staff", { name, role, serviceIds });
      onSaved(saved);
    } catch (err) {
  ...
  return (
    <Modal title={isEdit ? "Edit Staff" : "Add Staff"} onClose={onClose}>
      <form onSubmit={handleSubmit} className="flex flex-col gap-4">
        ...
        <FormSelect id="staffRole" label="Role" value={role} onChange={(e) => setRole(e.target.value as StaffRole)}>
          {STAFF_ROLES.map((r) => (
        ...
        <div className="flex flex-col gap-1.5">
          ...
          <div className="flex flex-col gap-2 rounded-lg border border-outline-variant p-3">
            {services.map((s) => (
              <label key={s.id} className="flex items-center gap-2 font-body-md text-body-md">
...
function SlotsTab({ staff }: { staff: StaffMember[] }) {
  ...
  const loadSlots = useCallback(async (id: string) => {
    ...
    try {
      const data = await api.get<TimeSlot[]>(`/providers/me/staff/${id}/slots`);
      setSlots(data);
    } finally {
  ...
  async function handleGenerate(e: FormEvent) {
    ...
    try {
      const res = await api.post<{ created: number; skipped: number }>(
        `/providers/me/staff/${staffId}/slots`,
        {
          ...form,
  ...
  if (staff.length === 0) {
    return (
      <p className="font-body-md text-body-md max-w-2xl py-8 text-on-surface-variant">
  ...
  return (
    <section className="max-w-2xl space-y-6">
      ...
      <form
        ...
        className="soft-card-shadow flex flex-col gap-4 rounded-xl border border-outline-variant bg-surface-container-lowest p-5"
      >
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <FormField
            ...
            onChange={(e) => setForm({ ...form, dailyStart: e.target.value })}
          />
          <FormField
            id="dailyEnd"
            label="Daily end"
            ...
            required
            value={form.dailyEnd}
            onChange={(e) => setForm({ ...form, dailyEnd: e.target.value })}
          />
          <FormField
            id="slotMinutes"
          ...
          <div className="flex flex-wrap gap-2">
            {WEEKDAYS.map((d) => (
              <button
                ...
                className={`font-label-md text-label-md rounded-full px-3 py-1.5 transition-colors ${
      ...
      <div>
        ...
        {loadingSlots ? (
          ...
          <p className="font-body-md text-body-md text-on-surface-variant">No upcoming slots yet.</p>
        ) : (
              ...
              <div key={day}>
                ...
                <div className="flex flex-wrap gap-2">
                  {daySlots.map((slot) => (
                    <span
                      key={slot.id}
                      className={`font-label-sm text-label-sm flex items-center gap-1 rounded-full px-3 py-1.5 ${
                    ...
                    >
                      ...
                      {slot.status === "AVAILABLE" && (
                        <button onClick={() => removeSlot(slot.id)} className="ml-1 text-on-surface-variant">
