  ...
  const todaysBookings = useMemo(
    () =>
      bookings
        ...
        .sort((a, b) => new Date(a.slot.startDatetime).getTime() - new Date(b.slot.startDatetime).getTime()),
    ...
    <ProviderLayout active="calendar">
          ...
          <div className="ml-4 hidden items-center gap-2 sm:flex">
            {FILTERS.map((f) => (
              <button
                ...
                className={`text-label-md rounded-full px-4 py-1.5 transition-all ${
                  filter === f.value
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex overflow-hidden rounded-lg border border-outline-variant">
              <button
                ...
                className="border-r border-outline-variant bg-surface p-2 text-on-surface-variant transition-colors hover:bg-surface-variant"
              >
                <span className="material-symbols-outlined">chevron_left</span>
              </button>
              <button
                onClick={() => setWeekStart(startOfWeek(new Date()))}
                className="text-label-md bg-surface px-4 py-2 font-bold transition-colors hover:bg-surface-variant"
              >
                Today
              </button>
              <button
                onClick={() => setWeekStart(addDays(weekStart, 7))}
                className="border-l border-outline-variant bg-surface p-2 text-on-surface-variant transition-colors hover:bg-surface-variant"
              >
          ...
          <div className="grid overflow-x-auto border-b border-outline-variant bg-surface-container" style={{ gridTemplateColumns: "80px repeat(7, minmax(90px, 1fr))" }}>
            ...
            {weekDays.map((day) => {
              ...
              return (
                <div
                  key={day.toISOString()}
                  className={`flex h-14 flex-col items-center justify-center border-l border-outline-variant ${
                ...
                >
                  ...
                  {isToday ? (
                    <div className="flex h-8 w-8 items-center justify-center rounded-full bg-primary text-body-lg font-bold text-on-primary">
                      {day.getDate()}
                    </div>
                  ) : (
                    <span className="text-body-lg font-bold">{day.getDate()}</span>
                  )}
                </div>
              );
            })}
          ...
          <div className="custom-scrollbar flex-1 overflow-auto">
            <div className="grid" style={{ gridTemplateColumns: "80px repeat(7, minmax(90px, 1fr))" }}>
              <div className="flex flex-col">
                {hours.map((h) => (
                  <div key={h} className="flex items-start justify-center border-b border-outline-variant pt-2" style={{ height: ROW_HEIGHT_PX }}>
                    <span className="text-xs font-bold text-on-surface-variant">{`${h.toString().padStart(2, "0")}:00`}</span>
                  </div>
                ))}
              </div>
              {weekDays.map((day) => (
                <div key={day.toISOString()} className="relative border-l border-outline-variant">
                  {hours.map((h) => (
                    <div key={h} className="border-b border-outline-variant" style={{ height: ROW_HEIGHT_PX }} />
                  ))}
                  {bookingsForDay(day).map((booking) => {
                    ...
                    return (
                      <div
                        key={booking.id}
                        className="absolute left-1 right-1 z-20 cursor-pointer rounded-xl border border-white/20 bg-primary-container p-3 text-on-primary-container shadow-md transition-transform hover:scale-[1.02]"
                        style={{ top: startOffset * ROW_HEIGHT_PX, height: durationHours * ROW_HEIGHT_PX }}
                      >
      ...
      <div className="glass-card fixed bottom-24 right-4 z-40 w-72 rounded-2xl border border-outline-variant/50 bg-white/80 p-6 shadow-2xl backdrop-blur-md sm:bottom-8 sm:right-8 sm:w-80 md:right-8">
        ...
        {loading ? (
          ...
          <p className="font-body-md text-body-md text-on-surface-variant">No appointments today.</p>
        ) : (
      ...
      {showWelcome && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-inverse-surface/40">
          <div className="soft-card-shadow w-full max-w-md rounded-xl bg-surface-container-lowest p-6">
            ...
            <div className="mt-6 flex gap-3">
              <button
                ...
                className="min-h-[48px] flex-1 rounded-lg border border-outline-variant bg-surface-container px-6 font-label-md text-label-md font-semibold text-on-surface"
              >
                Later
              </button>
              <Link
                ...
                onClick={() => setShowWelcome(false)}
                className="flex min-h-[48px] flex-1 items-center justify-center rounded-lg bg-primary px-6 font-label-md text-label-md font-semibold text-on-primary"
              >
                Set up services
              </Link>
            </div>
