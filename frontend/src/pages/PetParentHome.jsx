    ...
    return (<PetParentLayout active="home">
      ...
      <main className="flex flex-col gap-section-gap pt-4">
        <section className="flex flex-col gap-4">
          ...
          {loading ? (<p className="font-body-md text-body-md px-container-padding-mobile text-on-surface-variant">
              Loading...
            </p>) : (<div className="hide-scrollbar flex gap-gutter overflow-x-auto px-container-padding-mobile">
              {pets.map((pet) => (<Link key={pet.id} to={`/pets/${pet.id}`} className="group flex shrink-0 flex-col items-center gap-2">
              ...
              <button onClick={() => setAddPetOpen(true)} className="group flex shrink-0 flex-col items-center gap-2">
        ...
        <section className="px-container-padding-mobile">
          <Link to="/providers" className="group relative flex flex-col items-start overflow-hidden rounded-xl bg-primary p-5 text-left text-on-primary shadow-lg transition-transform active:scale-95">
      ...
      {addPetOpen && (<AddPetModal onClose={() => setAddPetOpen(false)} onCreated={(pet) => {
                setPets((prev) => [...prev, pet]);
                setAddPetOpen(false);
            }}/>)}

      {showWelcome && (<Modal title={`Welcome, ${firstName}!`} onClose={() => setShowWelcome(false)}>
          <p className="font-body-md text-body-md text-on-surface-variant">
          ...
          <div className="mt-6 flex gap-3">
            ...
            {firstPet && (<Link to={`/pets/${firstPet.id}?tab=records&upload=1`} onClick={() => setShowWelcome(false)} className="flex flex-1 items-center justify-center rounded-lg bg-primary px-6 font-label-md text-label-md font-semibold text-on-primary">
                Upload records
              </Link>)}
          </div>
        </Modal>)}
...
function AddPetModal({ onClose, onCreated }) {
    ...
    return (<Modal title="Add a Pet" onClose={onClose}>
