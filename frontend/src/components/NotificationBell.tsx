...
export function NotificationBell({ light = false }: { light?: boolean }) {
  ...
  useEffect(() => {
    api.get<Notification[]>("/notifications").then(setNotifications).catch(() => undefined);
  }, []);
  ...
  return (
    <div className="relative" ref={ref}>
      ...
      {open && (
        <div className="soft-card-shadow absolute right-0 top-12 z-30 w-80 rounded-xl border border-outline-variant bg-surface-container-lowest py-2">
            ...
            <div className="max-h-96 overflow-y-auto">
              {notifications.map((n) => (
                <button
                  ...
                  className={`flex w-full flex-col items-start gap-0.5 px-4 py-3 text-left transition-colors hover:bg-surface-container ${
                    n.isRead ? "opacity-60" : ""
                  }`}
                >
